...
func prodDERPMap(ctx context.Context, httpc *http.Client) (*tailcfg.DERPMap, error) {
	cfmt.Printf(`{{Warning: DERP relay servers have been disabled}}::red`)
	cfmt.Println()
	cfmt.Printf("DERP relay servers provide centralization which we do not support right now")
	cfmt.Println()
	cfmt.Printf("If you wish to use DERP relays, configure your relay and modify the config in ipn.prefs")
	cfmt.Println()
	cfmt.Println()
